...
describe("detectLang", () => {
  ...
  it("should match given tags case insensitively", () => {
    const localeUpper = Intl.DateTimeFormat().resolvedOptions().locale.toUpperCase();
  ...
  it("should match given sub-tags case insensitively", () => {
    const localeUpper = Intl.DateTimeFormat().resolvedOptions().locale.split("-")[0].toUpperCase();
    expect(detectLang(["example", localeUpper, "fr"])).toBe(localeUpper);
